...
$(function () {
  ...
  if (dt_customer_table.length) {
    var dt_customer = dt_customer_table.DataTable({
      ajax: {
        url: '/ProductF/CategoryEdit?handler=DataList', // Replace with your actual handler path
      ...
      columnDefs: [
...

        {
          ...
          render: function (data, type, full, meta) {
            ...
            var $imageUrl = full['imageFileUrl'];
            var $output = "";

            const jsClick = ` href="javascript:void(0);" onclick="previewImage('${$imageUrl}')" class="rounded-2">`;
            ...
            var $row_output =
              ...
              '/ProductF/ProductEdit/?ProductId=' + full['productId'] +
              '" class="text-primary text-decoration fw-medium">' +
              '<span class="text-truncate">' + full['productName'] + '</span>' +
              '</a>'  +
      ...
      buttons: [
        ...
        },
        {
          ...
          action: function () {
            window.open('/ProductF/ProductList', '_blank');
          }
